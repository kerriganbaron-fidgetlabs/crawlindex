...
import { countBlockersOf, getBlockersOf, getLatestStats } from '../../../lib/queries';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const blocked = await countBlockersOf(agent.token, agent.tier).catch(() => 0);
  return {
...
export default async function BotPage({ params }: Props) {
  ...
  const [blockers, blockedCount, stats] = await Promise.all([
    getBlockersOf(agent.token, agent.tier, 100),
    countBlockersOf(agent.token, agent.tier),
    getLatestStats(),
  ]);
  ...
  return (
    <>
      <PageHeader
        kicker={`${agent.operator} . tier ${agent.tier}`}
        title={agent.token}
        lede={agent.blurb}
      />
      ...
      <section className="mb-12">
        ...
        <p className="text-sm text-muted mb-4 max-w-2xl">
          ...
          it, so rules you set under <code className="font-mono">User-agent: *</code> do not carry
          over.
        </p>
      ...
      <section>
        ...
        <DomainTable rows={blockers} caption={`Domains that block ${agent.token}`} />
      </section>
    </>
  );
}
